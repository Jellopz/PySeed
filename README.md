...
RD.py works by generating such a big token that the CPU just has to work harder, and because there are so many different characters it fills storage pretty quick. RD.py is just a joke BTW because I wanted something to code and I had no idea what to make. Oh yeah and RD.py shares a lot in common with PySeed.py.
